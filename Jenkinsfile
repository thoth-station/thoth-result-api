...
pipeline {
    ...
    stages {
                ...
                script {                    
                    ...
                    openshift.withCluster() {
                        openshift.withProject(CI_TEST_NAMESPACE) {
                            if (!openshift.selector("template/result-api-buildconfig").exists()) {
                            ...
                            def model = openshift.process('result-api-buildconfig',
                                    "-p", 
        ...
        stage("Deploy to Test") {
            steps {
                script {
                    // aIStacksPipelineUtils.redeployFromImageStreamTag(CI_TEST_NAMESPACE, "result-api", "${env.TAG}")
                    // redeploy from ImageStreamTag ${env.TAG}
                    openshift.withCluster() {
                        openshift.withProject(CI_TEST_NAMESPACE) {
                            echo "Creating test tag from result-api:${env.TAG}"

                            openshift.tag("${CI_TEST_NAMESPACE}/result-api:${env.TAG}", "${CI_TEST_NAMESPACE}/result-api:test")

                            def result = null

                            deploymentConfig = openshift.selector("dc", "result-api")
        ...
        stage("Tag stable image") {
            steps {
                script {
                    ...
                    openshift.withCluster() {
                        openshift.withProject(CI_TEST_NAMESPACE) {
                            echo "Creating stable tag from result-api:${env.TAG}"

                            openshift.tag("${CI_TEST_NAMESPACE}/result-api:${env.TAG}", "${CI_TEST_NAMESPACE}/result-api:stable")
                        }
                    } // withCluster
                } // script
            } // steps
